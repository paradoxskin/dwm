...
#define ISTOP(C)                (C->tags & topmask)
#define ISVISIBLEONTAG(C, T)    ((C->tags & T)|ISTOP(C))
...
applyrules(Client *c)
{
    ...
    if (ch.res_name)
        XFree(ch.res_name);
    c->tags = c->tags & topmask ? topmask : c->tags & TAGMASK ? c->tags & TAGMASK : c->mon->tagset[c->mon->seltags];
}
        ...
        if (text[i] == '^' && !isCode) {
            ...
            while (text[++i] != '^') {
                if (text[i] == 'c') {
                    ...
                    memcpy(buf, (char*)text+i+1, 7);
                    buf[7] = '\0';
                    drw_clr_create(drw, &drw->scheme[ColBg], buf, 0x77);
                    i += 7;
                } else if (text[i] == 'd') {
                    drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
                    drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
...
showhide(Client *c)
{
    ...
    if (ISVISIBLE(c)) {
        ...
        if ((!c->mon->lt[c->mon->sellt]->arrange || c->isfloating) && !c->isfullscreen)
            resize(c, c->x, c->y, c->w, c->h, c->isfloating);
        showhide(c->snext);
    } else {
